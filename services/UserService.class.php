...
class UserService {
	...
	private function thirdLogin($username, $headUrl, $openId, $accessToken, $auth_type) {
		
		$sqlHelper = new SQLHelper();
		// 检查openId是否已经记录过
		$sql = "select up.*, ua.* from user_profile up, user_auth ua where ua.open_id='" . $openId . "' and up.id=ua.user_id and ua.auth_type=" . $auth_type;
		$userArr = $sqlHelper->execute_dql_array($sql);
		if(count($userArr) > 1) {
			return false;
			...
			if($user["username"] != $username ||
				...
				if($user["username"] == $username && $user["head_url"] == $headUrl) {
					$sql = "update user_auth set access_token='" . $accessToken . "' where open_id='" . $openId . "' and auth_type=" . $auth_type;
				} else {
					$sql = "update user_profile up, user_auth ua set up.username='" . $username . "', up.head_url='" . $headUrl . "', ua.access_token='" . $accessToken . "' where ua.open_id='" . $openId . "' and ua.auth_type=" . $auth_type . " and up.id=ua.user_id";
				}
			...
			$userId = $sqlHelper->getLastInsertedId();
			// 添加认证信息
			$sql = "insert into user_auth (gmt_create, gmt_modify, user_id, auth_type, open_id, access_token) values (now(), now(), " . $userId . ", " . $auth_type . ", '" . $openId . "', '" . $accessToken . "')";
			$res = $sqlHelper->execute_dqm($sql);
			if($res != 1) {
				return false;
			}
	...
	public function weiboLogin($username, $headUrl, $openId, $accessToken) {
		return $this->thirdLogin($username, $headUrl, $openId, $accessToken, 1);		
	}
	
	public function qqLogin($username, $headUrl, $openId, $accessToken) {
		return $this->thirdLogin($username, $headUrl, $openId, $accessToken, 2);
	}
	
	public function githubLogin($username, $headUrl, $openId, $accessToken) {
		return $this->thirdLogin($username, $headUrl, $openId, $accessToken, 3);
	}
